...
p. In order to create a template, elements that can be edited by the user are defined using "freemarker":http://freemarker.org/ tokens.
...
**** %(#dynattrtempexpr)*template-expression*% : the name of the template token. It is the freemarker token without the dollar sign.
...
* insert the freemarker token *${sfxCallback}* inside a *script* code block
* insert the freemarker token *${sfxContentDefiner}* inside a *script* block
...
h3. Valid freemarker tokens

p. The following freemarker tokens are available for SlideshowFX:
