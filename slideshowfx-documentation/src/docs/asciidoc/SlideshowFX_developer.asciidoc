...
===== SlideshowFXPublisherPlugin

Gradle plugin to be applied for projects that need to be published on Bintray.
...
|===
| Name | Description
| publish |  Publishes the artifact on Bintray
| listSnapshotProjects | Lists all projects having a snapshot version
|===
...
|===
| Name | Description | Default value | Used by tasks
| bintrayUserName | Username to publish on Bintray. Can be replaced by a `BINTRAY_USER_NAME` environment variable | - | `publish`
| bintrayApiKey | API key to publish on Bintray. Can be replaced by a `BINTRAY_API_KEY` environment variable | - | `publish`
|===
